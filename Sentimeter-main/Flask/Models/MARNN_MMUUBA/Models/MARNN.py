...
class MARNN:
    ...
    def __init__(self, feat_ext_path):
        ...
        text_data, audio_data, video_data, labels, mask = self.preprocess_data(feat_ext_path)

        # self.train_text, self.dev_text, self.test_text = text_data
        # self.train_audio, self.dev_audio, self.test_audio = audio_data
        # self.train_video, self.dev_video, self.test_video = video_data
        # self.train_label, self.dev_label, self.test_label = labels
        # self.train_mask, self.dev_mask, self.test_mask = mask
        ...
        # Hna b3ml instance mn el keras tensor >>> 3chan a3ml el model fe keras lazem adeelo inputs we elly ana
        # mstneeh mno k output
        ...
        # hna b3ml masking we de bst5dmha lw msln fe 7aga 3ayz afwtha fe el data >>>> hna hfwt ay data qemetha b zero
    ...
    def preprocess_data(self, feat_ext_path):
        
        INPUT_DIR = os.path.join(feat_ext_path, os.pardir, 'input')
        # Retreive Data
        ...
        # hbd2 a2sm el training set le cross validation (aw develpment set y3ny) 20% we training set 80%
        ...
        # text_data = [train_text, dev_text, test_text]
        # audio_data = [train_audio, dev_audio, test_audio]
        # video_data = [train_video, dev_video, test_video]
        # labels = [train_label, dev_label, test_label]
        # mask = [train_mask, dev_mask, test_mask]
    ...
    def AttentionBasedGRU(self, AD):
        ...
        Pt = Dropout(self.dropDense)(TimeDistributed(Dense(100, activation='tanh'))(H))
    ...
    def train(self, runs, lr):
        ...
        for j in range(runs):
            ...
            reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor= 'val_loss',\
                                  factor= 0.1, patience= 20, min_lr= 1e-5)
